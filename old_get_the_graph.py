...
def forming(method_of_interest):
	...
	cursor.execute("SELECT line_no, type, thread_id, direction, method, args FROM log LIMIT {}".format(interested_line_number))
	data = []
	...
	for i in range(0, len(data)):
		if(data[i]['Thread_ID'] == interested_thread_id):
			if(data[i]['Type'] == 'Method' and data[i]['Direction'] == ">"):
				line_no.append(p)
				...
				p = p + 1
			elif(data[i]['Type'] == 'API'):
			# elif(data[i]['Type'] == 'API' and data[i]['Direction'] == ">"):
				line_no.append(p)
				type_of_function.append(data[i]['Type'])
				direction.append(data[i]['Direction'])
				method.append(data[i]['Method'])
